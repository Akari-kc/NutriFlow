...
class ReportController extends Controller
{
    ...
    public function exportCsv(Request $request): StreamedResponse
    {
        ...
        $callback = function() use ($data) {
            ...
            fputcsv($handle, []);
            fputcsv($handle, ['Learner ID', 'Name', 'LRN', 'Grade', 'Section', 'Classification', 'Latest Measurement', 'Assessment Phase', 'BMI', 'Weight kg', 'Height cm', 'Meals Served']);

            foreach ($data['reportRows'] as $row) {
                fputcsv($handle, [
                    $row['student']->learner_uid,
                    $row['student']->name ?? '',
                    $row['student']->lrn ?? '',
                    ...
                    $row['status'],
                    $row['measured_at'] ?? '',
                    $row['student']->latestMeasurement?->assessment_phase ?? '',
                    $row['bmi'] ?? '',
                    $row['weight_kg'] ?? '',
                    $row['height_cm'] ?? '',
                    $row['meals_served'],
