        ...
        @php
          ...
          if (auth()->check()) {
              $topbarSuggestions = \App\Models\Student::when($topbarSchoolId, fn($q) => $q->where('school_id', $topbarSchoolId))
                ->orderBy('name')
                ->limit(80)
                ->get(['learner_uid', 'name', 'source_learner_reference', 'class_name', 'section'])
                ->flatMap(fn($student) => [$student->display_name, $student->learner_uid, $student->class_name, $student->section])
